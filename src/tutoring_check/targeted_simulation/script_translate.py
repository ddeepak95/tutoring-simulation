...
from tutoring_check.targeted_simulation.script import _DATA_DIR, ScriptError, load_script
...
def translate_script(
    ...
    *,
    ...
    """Write `data/scripts/<language_id>/<script_id>.json` from the English script."""
    root = scripts_root or _DATA_DIR / "scripts"
    source = load_script(script_id, SOURCE_LANGUAGE_ID, root)
    ...
    turns, refinements = translate_conversation(
        json.dumps([t.text for t in source.conversation], ensure_ascii=False),
        len(source.conversation),
        target_language,
        mode,
        ts,
    )
        ...
        json.dumps(
            {
                ...
                "conversation": [
                    {"speaker": turn.speaker, "text": text}
                    for turn, text in zip(source.conversation, turns)
                ],
            },
            ensure_ascii=False,
            indent=2,
    ...
    load_script(script_id, language_id, root)
    return out_path


def source_script_ids(scripts_root: Path | None = None) -> list[str]:
    """Every script authored in English."""
    root = (scripts_root or _DATA_DIR / "scripts") / SOURCE_LANGUAGE_ID
