"""Exploratory paired word-count comparisons using pairwise on-topic matching."""
import json
...
def sign_test(values):
    ...
    n=len(nonzero);positive=sum(x>0 for x in nonzero)
    p=min(1.0,2*sum(comb(n,k) for k in range(min(positive,n-positive)+1))/2**n) if n else 1.0
    return dict(n=n,positive=positive,negative=n-positive,ties=len(values)-n,p=p)

def holm(rows,field):
    ordered=sorted(rows,key=lambda r:r[field]['p']);previous=0
    for i,row in enumerate(ordered):
        previous=max(previous,min(1.0,(len(rows)-i)*row[field]['p']))
        row[field]['p_holm']=previous

def main():
    d=json.loads((ROOT/'comparison.json').read_text(encoding='utf-8'))
    candidates=sorted({(r['topic'],r['model']) for r in d['rows']})
    lookup={(r['topic'],r['model'],r['condition']):r['word_count'] for r in d['rows']}
    results=[]
    ...
    for r in results:
        lines.append(f"| {r['comparison']} | {r['pair_count']} | {r['pair_sign_test']['n']} | {r['topic_count']} | {r['mean_delta_words']:.1f} | {r['median_delta_words']:.1f} | {r['pair_sign_test']['p']:.6g} | {r['pair_sign_test']['p_holm']:.6g} | {r['topic_sign_test']['p']:.6g} | {r['topic_sign_test']['p_holm']:.6g} |")
    ...
    (ROOT/'word_count_statistics.md').write_text('\n'.join(lines),encoding='utf-8')
    print('\n'.join(lines[6:]))
