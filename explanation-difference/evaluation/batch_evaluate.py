...
def work(source):
    ...
    language='English' if job['run_index']==1 else 'Tamil'
    ...
    return dict(source=str(source),output=str(output),success=result.returncode==0,model=job['model'],topic=job.get('topic_name') or source.parent.name.split('_',1)[1].replace('_',' '),condition=job['run_index'],language=language)
...
def main():
    sources=[p for p in sorted(ROOT.glob('ch-*/run-*.json')) if json.loads(p.read_text(encoding='utf-8')).get('status')=='completed']
    rows=[]
